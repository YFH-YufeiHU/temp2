...
def adjust_learning_rate(model, lr):
...
def main():
    ...
    train_queue, model = build_fn(model_state_dict,optimizer_state_dict)
    ...
    for epoch in range(args.epochs):
        ...
        for i, (images, labels) in enumerate(train_queue):
            i_iter += 1
            optimizer = adjust_learning_rate(model, lr_poly(args.lr_max, i_iter, total_iter, args.lr_schedule_power))
